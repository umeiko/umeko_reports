#!/usr/bin/env bash
# Evaluate one finished 143M run: dev BPB at steps 0,632,...,10112 + test BPB
# at the final step, same offline protocol as the 43M reproduction.
# Usage: eval_run_143m.sh <run_name> <seed> <npu_logical_id>
set -eo pipefail
...
RUN=$MZY/bilingual_npu_cluster/training/runs_143m/$RUN_NAME
POOL=$MZY/umeko_reports/data/materialized_331m_143m
INIT_HF=$MZY/bilingual_npu_cluster/training/init_hf_143m/init-s$SEED
EVAL_DIR=$RUN/evals
mkdir -p "$EVAL_DIR"
